...
if (os === 'windows') {
  ...
  if (!directory) {
    if (process.env.RUNTIME_UNSIGNED_BUILD !== '1')
      throw new Error('Centrally signed runtime bytes are required for publication.');
  } else {
    const name = `engram-windows-${arch}.exe`;
    const manifest = JSON.parse(
      readFileSync(path.join(directory, 'signed-runtime-manifest.json'), 'utf8'),
    );
    const entry = manifest.files.find(file => file.name === name);
    if (
      !entry ||
      executableHash(path.dirname(target), path.basename(target)) !== entry.unsignedSha256 ||
      executableHash(directory, name) !== entry.signedSha256
    ) {
      throw new Error(
        'Rebuilt executable does not match the source bytes authenticated by central signing.',
      );
    }
